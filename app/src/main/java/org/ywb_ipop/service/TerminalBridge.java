...
import org.ywb_ipop.TerminalView;
import org.ywb_ipop.bean.HostBean;
import org.ywb_ipop.bean.PortForwardBean;
import org.ywb_ipop.bean.SelectionArea;
import org.ywb_ipop.transport.AbsTransport;
...
public class TerminalBridge implements VDUDisplay {
    ...
    public int isendFlag = 0;//0:stop,1:run,2:pause
    ...
    //记录存盘
    public boolean bFilesave=false;
    ...
    protected BridgeSendOverListener sendoverListener = null;

    private int iSendtype=0;
    private int bCrlf=1,iLoop=1;

    public void startSend(String SendTxt,int timeperiod,int sendtype,int bCrlf1,int iLoop1)
    {
        String pattern = "\n";
        Pattern pat = Pattern.compile(pattern);
        sendStr = pat.split(SendTxt);
        sendLine = sendStr.length;
        period=timeperiod;
        iSendtype=sendtype;
        bCrlf=bCrlf1;
        iLoop=iLoop1;
    ...
    public void stopSend(boolean truestop)
    {
        stopTimer();
                    ...
                    case SEND_TXT_MESSAGE:
                        ...
                        if(sendLine>0) {
                            if (sendLoc >=sendLine)
                            {
                                ...
                                if(iLoop==0) {
                                    stopSend(true);
                                    if(sendoverListener!=null)
                                        sendoverListener.onSendOvered(TerminalBridge.this);
                                   /* if(manager.defaultBridge.)
                                    {
                                        ImageButton sendbutton = (ImageButton) parent.findViewById(R.id.sendButton);
                                        sendbutton.setImageDrawable(manager.getResources().getDrawable(R.drawable.play));
                                        ImageButton pausebutton = (ImageButton) parent.findViewById(R.id.pauseButton);
                                        pausebutton.setImageDrawable(manager.getResources().getDrawable(R.drawable.pause));
	...
	protected void startConnection() {
        ...
        if(host.getProtocol().equals("bluetooth"))
			outputLine(manager.res.getString(R.string.ywbblue_connecting, host.getUsername(),  host.getHostname()));
		else
	...
	public void onConnected() {
        if(bFilesave) {
            try {
                ...
                String temps = host.getNickname().replace('@', '_');
                temps = temps.replace('.', '_') + "_";
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(manager.getApplicationContext());
                //设保存文件
                String tempdir = prefs.getString("send_autosavedir", Environment.getExternalStorageDirectory().toString());
                String fileName = tempdir + "/" + temps + sDateFormat.format(new java.util.Date()) + ".txt";


                fsave = new FileOutputStream(fileName);
            } catch (Exception e) {
            }
        }
    ...
    public void setOnSendOveredListener(BridgeSendOverListener sendoverListener) {
        this.sendoverListener = sendoverListener;
    }
	/**
	...
	public void propagateConsoleText(char[] rawText, int length) {
        try {
         String temps=new String(rawText,0,length);
           // Log.d("ywb:","【"+temps+"】");
            if(bFilesave)
            fsave.write(temps.getBytes());
    }catch(Exception e){}
		if (parent != null) {
