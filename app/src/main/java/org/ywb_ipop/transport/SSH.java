...
import org.ywb_ipop.bean.PubkeyBean;
import org.ywb_ipop.service.TerminalBridge;
import org.ywb_ipop.service.TerminalManager;
import org.ywb_ipop.service.TerminalManager.KeyHolder;
import org.ywb_ipop.util.HostDatabase;
import org.ywb_ipop.util.PubkeyDatabase;
import org.ywb_ipop.util.PubkeyUtils;
import org.ywb_ipop.util.MyEncode;
   ...
   // }
    ...
    public String getPass(SQLiteDatabase db,String name)
    {
        ...
        if(recnum>0) {
            ...
            //temps=c.getString(2);
            try {
                temps = MyEncode.decrypt("IPOPEncode", c.getString(2));
            }catch(Exception e){
                temps="";
            }
   ...
   public void savePass(SQLiteDatabase db,String name,String passwd)
    {
            ...
            if(passwd.equals(""))
             temps="";
                ...
                try {
                temps = MyEncode.encrypt("IPOPEncode", passwd);
                }catch(Exception e){
                    temps="";
                }
            }
		...
		try {
			...
			if (!pubkeysExhausted &&
				...
				if (password != null
                    ...
                    try {
                    SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase("/data/data/org.ywb_ipop/databases/hosts", null);
                    savePass(db,host.getNickname(),password);
                     db.close();
                  // HostDatabase hostdb;

                //  hostdb = new HostDatabase(context);

                //  hostdb.savePass(host.getNickname(),password);
                  //  hostdb.close();
                    }catch (Exception e) {
