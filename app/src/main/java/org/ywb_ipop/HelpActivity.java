...
public class HelpActivity extends Activity {
    ...
    public   String HELPDIR="help";//getResources().getString(R.string.ywb_help);
	...
	public void onCreate(Bundle icicle) {
        ...
        temps=getResources().getString(R.string.ywb_help);
        HELPDIR=temps;
			...
			for (String name : am.list(HELPDIR)) {
				if (name.endsWith(SUFFIX)) {
                    ...
                    if(HELPDIR.equals("help_cn")) {
                        if(name.equals("Hints.html"))
                            temps="小提示";
                        else if(name.equals("PhysicalKeyboard.html"))
                            temps="物理键盘";
                        else if(name.equals("ScreenGestures.html"))
                            temps="屏幕手势";
                        else if(name.equals("VirtualKeyboard.html"))
                            temps="虚拟键盘";
                        else
                            temps = name.substring(0, name.length() - SUFFIX.length());
                    }
                    else
					temps = name.substring(0, name.length() - SUFFIX.length());
